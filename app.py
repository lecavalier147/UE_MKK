...
# ------------------ АВТОРИЗАЦИЯ (из st.secrets) ------------------
# Конфигурация загружается из секретов

config = {
    'credentials': st.secrets["auth"]["credentials"],
    'cookie': st.secrets["auth"]["cookie"],
    'pre-authorized': st.secrets["auth"]["pre-authorized"]
}
...
# ------------------ ФУНКЦИИ ДЛЯ СЦЕНАРИЕВ В SUPABASE ------------------
...
# ------------------ ОСНОВНОЙ БЛОК ПАРАМЕТРОВ (виджеты с key) ------------------
with st.sidebar:
...
def calculate_loan(params, is_new=True):
    ...
    revenue_breakdown = {
        'Процентный доход': interest, 'Комиссия за выдачу': fee_income,
        'Кросс-продукты': cross_income, 'Страховки': ins_income,
        'Пролонгация': prolong_income, 'Штрафы': penalty,
        'Продажа портфеля': portfolio_sale, 'Комиссия за погашение (доход)': repay_fee_inc_amount,
        'Потеря от досрочки': -early_loss,
    }
    if is_new:
        revenue_breakdown['Отказной трафик'] = lead_price / TR * ((1 - AR) / AR)
    cost_breakdown = {
        'CAC (прямой)': cac_direct, 'СМС': sms_cost, 'Колл-центр': kc_cost,
        'Скоринг (на заём)': scoring_per_loan, 'Идентификация (на заём)': ident_per_loan,
        'Перевод денег': money_transfer, 'Взыскание': collection,
        'Фондирование': funding, 'Комиссия за погашение (расход)': repay_fee_exp_amount,
...
profit_new, rev_new, cost_new, _ = calculate_loan(params_new, is_new=True)
if use_repeat and params_repeat:
    profit_repeat, rev_rep, cost_rep, _ = calculate_loan(params_repeat, is_new=False)
    ...
    profit_repeat = 0; ltv = profit_new
...
fig1 = px.bar(df_rev_new, x="Статья", y="Сумма (₽)", title="Структура доходов (новый заём)")
st.plotly_chart(fig1, use_container_width=True)
fig2 = px.bar(df_cost_new, x="Статья", y="Сумма (₽)", title="Структура расходов и потерь (новый заём)")
st.plotly_chart(fig2, use_container_width=True)

st.caption("Модель включает новый и повторный займы, CAC, отказной трафик, продажу просрочки, комиссии за погашение.")